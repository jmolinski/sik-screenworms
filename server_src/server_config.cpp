...
namespace {
    ...
    constexpr uint16_t DEFAULT_ROUNDS_PER_SEC = 50;
    ...
    constexpr uint8_t MAX_TURNING_SPEED = 90;
    constexpr uint16_t MIN_ROUNDS_PER_SEC = 1;
    constexpr uint16_t MAX_ROUNDS_PER_SEC = 250;
    constexpr uint16_t MIN_BOARD_DIM_SIZE = 16;
    constexpr uint16_t MAX_BOARD_DIM_SIZE = 4096;
} // namespace
...
ServerConfig::ServerConfig(std::unordered_map<char, std::string> params)
    ...
    if ((it = params.find('s')) != params.end()) {
        rngSeed = utils::optionalParamValueToUint<uint32_t, 1, std::numeric_limits<uint32_t>::max()>(it);
    }
    ...
    if ((it = params.find('v')) != params.end()) {
        roundsPerSec = utils::optionalParamValueToUint<uint16_t, MIN_ROUNDS_PER_SEC, MAX_ROUNDS_PER_SEC>(it);
    }
